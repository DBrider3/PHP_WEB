<?php
  ...
  require_once $_SERVER['DOCUMENT_ROOT'].'/dbcon.php';
    ...
    if($is_logged=='YES') {
      ...
      if ($user_id1 == 'admin' && $_SESSION['is_admin']==1){
        $message = $user_id1 . ' , 관리자 로그인 했습니다.';
        // header("Location: admin.php");
      }
      else{
        $message = $user_id1 . ' 님, 로그인 했습니다.';
	...
	if(isset($_GET['edit_id']) && !empty($_GET['edit_id']))
	{
		...
		$stmt_edit = $connection->prepare('SELECT * FROM users WHERE id = :user_id');
	...
	if( ($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['btn_save_updates']))
	{
	   ...
	   $password = $row['password'];
		...
		if(!isset($errMSG))
		{
			$stmt = $connection->prepare('UPDATE users SET password=:user_password, userprofile=:userprofile, activate=:activate WHERE id=:user_id');
...
<body>
  ...
  <form id="myform" method="post" enctype="multipart/form-data" class="form-horizontal" style="margin: 0 300px 0 300px;border: solid 1px;border-radius:4px">
    ...
    <tr>
        ...
        <input type="hidden" name="__autocomplete_fix_<? echo $r1; ?>" value="editid" />
        </td>
    </tr>
    <tr>
